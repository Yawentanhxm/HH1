...
{
	Inparamss = new ArrayList ();
	Outparamss = new ArrayList ();
	...
	foreach (OperationMessage opm in oper.Messages)
	{
		if (opm is OperationInput)
			Buildparamseters (Inparamss, opm);
		else if (opm is OperationOutput)
			Buildparamseters (Outparamss, opm);
	}
	...
	InputparamssRepeater.DataSource = Inparamss;
	InputFormparamssRepeater.DataSource = Inparamss;
	OutputparamssRepeater.DataSource = Outparamss;
}

void Buildparamseters (ArrayList list, OperationMessage opm)
{
	Message msg = descriptions.GetMessage (opm.Message);
	if (msg.Parts.Count > 0 && msg.Parts[0].Name == "paramseters")
	{
...
static string keywords_cs =
	...
	"\\bbyte\\b|\\bfixed\\b|\\boverride\\b|\\btry\\b|\\bcase\\b|\\bfloat\\b|\\bparamss\\b|\\btypeof\\b|\\bcatch\\b|\\bfor\\b|" +
...
static string keywords_vb =
	...
	"\\bparamsArray\\b|\\bPreserve\\b|\\bPrivate\\b|\\bProperty\\b|\\bProtected\\b|\\bPublic\\b|\\bRaiseEvent\\b|\\bReadOnly\\b|" +
...
<head runat="server">
    ...
    <style type="text/css">
		...
		.paramsTable { font-size: x-small }
		.paramsTable TR { background-color: gainsboro }
		.paramsFormTable { font-size: x-small; padding: 10px; background-color: gainsboro }
		.paramsFormTable TR { background-color: gainsboro }
		.paramsInput { border: solid 1px gray }
...
-->
	...
	<% if (CurrentTab == "main") { %>
		<span class="label">Input paramseters</span>
		<div class="smallSeparator"></div>
		<% if (Inparamss.Count == 0) { %>
			No input paramseters<br>
		<% } else { %>
			<table class="paramsTable" cellspacing="1" cellpadding="5">
			<asp:repeater id="InputparamssRepeater" runat=server>
				<itemtemplate>
					<tr>
		...
		<% if (Outparamss.Count > 0) { %>
		<span class="label">Output paramseters</span>
			<div class="smallSeparator"></div>
			<table class="paramsTable" cellspacing="1" cellpadding="5">
			<asp:repeater id="OutputparamssRepeater" runat=server>
				<itemtemplate>
					<tr>
					<td width="150"><%#DataBinder.Eval(Container.DataItem, "Name")%></td>
					<td width="150"><%#DataBinder.Eval(Container.DataItem, "Type")%></td>
...
-->
	...
	<% if (CurrentTab == "test") { 
		if (CurrentOperationSupportsTest) {%>
			Enter values for the paramseters and click the 'Invoke' button to test this method:<br><br>
			...
			<table class="paramsFormTable" cellspacing="0" cellpadding="3">
			<asp:repeater id="InputFormparamssRepeater" runat=server>
				<itemtemplate>
					<tr>
					<td><%#DataBinder.Eval(Container.DataItem, "Name")%>:&nbsp;</td>
					<td width="150"><input class="paramsInput" type="text" size="20" name="<%#DataBinder.Eval(Container.DataItem, "Name")%>"></td>
					</tr>
				</itemtemplate>
			</asp:repeater>
		...
		<% } else {%>
		The test form is not available for this operation because it has paramseters with a complex structure.
		<% } %>
	<% } %>
